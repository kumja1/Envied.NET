...
namespace Envied.Common.Extensions;
    internal static class SpanExtensions
    {
        ...
        public static void Sort(this Span<string> span, int index, int length, IComparer<string> comparer)
        {
            if ((uint)index >= (uint)span.Length || (uint)length > (uint)(span.Length - index))
                throw new ArgumentOutOfRangeException();

            if (length > 1)
                HybridSort(span.Slice(index, length), comparer);
        }
